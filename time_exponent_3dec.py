...
for fp in files_q0:
    if not fp.exists():
        ...
        continue
    results_q0.append(analyze_run(fp, mesh_size=4.0))
...
for fp in files_wi:
    if not fp.exists():
        print(f"Missing: {fp.name}")
        continue
    r = analyze_run(fp, mesh_size=1.0)
    if "w_i" not in r:
...
r = analyze_run(result_dir / "run-wi-1e-05.hdf5", mesh_size=1.0)
...
def fit_velocity_for_run(fp, buffer=1.0):
    r = analyze_run(fp, mesh_size=buffer)
