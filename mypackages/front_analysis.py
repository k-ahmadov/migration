...
class FrontResults:
    ...
    A_emp: Prefactor
    α_emp: ScalingExponent

    def x_empirical(self) -> XPositions:
        return self.A_emp * self.t_front**self.α_emp
...
class FrontResultsWithAnalytical(FrontResults):
    A_ana: Prefactor
    α_ana: ScalingExponent
    ζ_front: float

    def x_analytical(self) -> np.ndarray:
        return self.A_ana * self.t_front**self.α_ana
...
def analyze(
    ...
    A_emp, α_emp = physics.fit_front_power_law(t_front, x_front)
    return FrontResults(t_front=t_front, x_front=x_front, A_emp=A_emp, α_emp=α_emp)


def _compute_zeta_front_rigid(
    x_front: np.ndarray, t_front: np.ndarray, D: float, alpha: float
) -> float:
    """Estimate the dimensionless front amplitude ζ from front positions."""
    mask = t_front != 0
    if not np.any(mask):
        raise ValueError("No non-zero front times — cannot estimate ζ.")
...
def analyze_rigid(
    ...
    α_ana = RIGID_DIFFUSION_EXPONENT
    ζ = _compute_zeta_front_rigid(result.x_front, result.t_front, D, α_ana)
    return FrontResultsWithAnalytical(
        **vars(result), A_ana=ζ * D**α_ana, α_ana=α_ana, ζ_front=ζ
    )
...
def analyze_soft(
    ...
    M = physics.parameter_M(run.params)
    α_ana = SOFT_DIFFUSION_EXPONENT
    ζ = _compute_zeta_front_soft(result.x_front, result.t_front, M, q, α_ana)
    A_ana = ζ * (M * q**3) ** (1 / 5)
    return FrontResultsWithAnalytical(
        **vars(result), A_ana=A_ana, α_ana=α_ana, ζ_front=ζ
    )
